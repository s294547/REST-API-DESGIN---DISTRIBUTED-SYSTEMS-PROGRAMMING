...
/**
 * Delete a public film having filmId as ID. If the film has reviews, drafts and agreements they are all deleted.
 *
 * Input: 
 * - filmId: the ID of the film that needs to be deleted
 * - owner: ID of the user who is deleting the film
    ...
    db.serialize(function(){
        ...
        db.all(sql1, [filmId], (err, rows) => {
            if (err){
                ...
                const sql2 = 'DELETE FROM agreements WHERE id IN (SELECT i.id FROM draftIds i WHERE i.filmId= ?)';
                db.run(sql2, [filmId], (err) => {
                    if (err){
                        ...
                        const sql3 = 'DELETE FROM drafts WHERE id IN (SELECT i.id FROM draftIds i WHERE i.filmId = ?)';
                        db.run(sql3, [filmId], (err) => {
                            if (err){
                                db.run("ROLLBACK;");
                                reject(err);
                              ...
                              const sql4 = 'DELETE FROM draftIds WHERE filmId = ?';
                              db.run(sql4, [filmId], (err) => {
                                  if (err){
                                    ...
                                    reject(err);
                                      ...
                                      const sql5 = 'DELETE FROM reviews WHERE filmId = ?';
                                      db.run(sql5, [filmId], (err) => {
                                          if (err){
                                              ...
                                              const sql6 = 'DELETE FROM films WHERE id = ?';
                                              db.run(sql6, [filmId], (err) => {
                                                  if (err){
                                                    db.run("ROLLBACK:");
                                                    reject(err);
                                                  }
 ...
 **/
 exports.getInvitedFilms = function(req) {
    return new Promise((resolve, reject) => {
      ...
      var sql = "SELECT DISTINCT f.id as fid, f.title, f.owner, f.private, f.watchDate, f.rating, f.favorite, c.total_rows FROM films f, reviews r, (SELECT count(DISTINCT f2.id) total_rows FROM films f2, reviews r2 WHERE f2.private=0 AND f2.id = r2.filmId AND r2.reviewersId LIKE ?) c WHERE  f.private = 0 AND f.id = r.filmId AND r.reviewersId LIKE ?"
      var limits = getPagination(req);
      var reviewersId='%-'+req.user.id+'-%';
      if (limits.length != 0) sql = sql + " LIMIT ?,?";
      limits.unshift(reviewersId);
      limits.unshift(reviewersId);
 ...
 **/
 exports.getInvitedFilmsTotal = function(reviewerId) {
    return new Promise((resolve, reject) => {
        var sqlNumOfFilms = "SELECT count(DISTINCT f.id) total FROM films f, reviews r WHERE  f.private = 0 AND f.id = r.filmId AND r.reviewersId LIKE ? ";
